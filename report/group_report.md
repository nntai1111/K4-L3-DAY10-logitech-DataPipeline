...
| B3 Pytest | Bộ test trong `tests/` chạy trên thư mục tạm, không ghi vào `data/` thật, provider LLM ép về `mock`, `run_date` cố định. Kết quả `bash script/run_tests.sh` ngày 2026-09-25: 252 test pass trong 54.47 giây, coverage `src/` 99% (1081 câu lệnh, thiếu 6). Test end-to-end chạy `phase1` một lần và `corruption_flow` hai lần trong project tạm. Workflow `.github/workflows/tests.yml` chạy bộ test trên Python 3.11 | `tests/`, `script/run_tests.sh` |
