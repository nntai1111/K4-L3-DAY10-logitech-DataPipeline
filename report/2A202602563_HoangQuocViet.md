...
| --- | --- | --- |
| Tự làm một bản pipeline đầy đủ thứ hai trên nhánh riêng `feat/viet-pipeline`: nạp snapshot Crossref, cleaning, gate GX 1.x + freshness SLA, 6 kịch bản corruption, repair idempotent, test set 10 câu và các báo cáo. Nhóm làm theo cách mỗi người tự dựng một bản chạy được trước, rồi chọn phần tốt nhất để ghép. | Cả nhóm, lúc so sánh và chọn phần ghép vào main | Nhóm có thêm một bản để đối chiếu. Hai điều tôi rút ra từ bản này có trong `quality.py` trên main: check `source_papers_present` (mục 5) và cách ghép kết quả GX theo `check_id` (mục 6). Code của bản này không phải code nộp. |
| Làm một app demo Streamlit chạy trên bản của tôi: agent trợ lý nghiên cứu trả lời trên ChromaDB, tab "Silent failure" đặt dữ liệu sạch và dữ liệu hỏng cạnh nhau, tab observability, và phần nạp trực tiếp (agent lấy bài mới từ Crossref, bài chỉ được index sau khi qua quality gate). | Cả nhóm, làm công cụ minh hoạ (ngoài code nộp) | Cho thấy trực quan hai ý chính của bài: dữ liệu hỏng vẫn cho ra câu trả lời trông bình thường, và gate phải đứng trước vector store. App không nằm trên main. |
...
Báo cáo gate gắn sai tên check vì Great Expectations 1.x không trả kết quả theo thứ tự expectation. Lỗi này tôi gặp ở bản làm riêng của mình.
...
1. **Về data pipeline:** pipeline chạy trên snapshot vẫn "cũ đi" theo lịch dù dữ liệu không đổi, vì `age_days` tính theo ngày chạy. Trên main, baseline chạy ngày 2026-09-25 đã có 1/24 bài stale (181 ngày). Ở bản làm riêng, tôi cố định ngày chạy bằng biến `RUN_DATE` để chạy lại vẫn ra cùng báo cáo. Môi trường cũng là một phần của khả năng chạy lại: trên máy Windows có Application Control, một DLL trong wheel scikit-learn bị chặn khi import; tôi pin một phiên bản đã biết chạy được trong venv thay vì tắt tính năng bảo mật đó.
