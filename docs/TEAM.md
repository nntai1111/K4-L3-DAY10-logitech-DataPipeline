...
- **Công việc chi tiết đã hoàn thành:**
  ...
  - Bonus B1: `app/` (Streamlit) gồm ba tab: trợ lý nghiên cứu hỏi trên một collection, tab "Silent failure" đặt cùng một câu hỏi lên ba collection baseline/corrupted/repaired và hiện câu trả lời cạnh nhau, và trang quan sát dữ liệu đọc artifact. Bonus B2: auto-repair khi gate fail. Bonus B3: bộ pytest trong `tests/` (249 test, coverage `src/` 99%), chạy một lệnh bằng `bash script/run_tests.sh`, có workflow GitHub Actions chạy trên Python 3.11.
- **Điều học được / Đóng góp chính:**
